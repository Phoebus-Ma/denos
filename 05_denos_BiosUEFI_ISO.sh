...
grub-mkstandalone \
    --format=x86_64-efi \
    --output=$PWD/$LIVE_BOOT/scratch/bootx64.efi \
    --locales="" \
    --fonts="" \
    "boot/grub/grub.cfg=$PWD/$LIVE_BOOT/scratch/grub.cfg"

(cd $PWD/$LIVE_BOOT/scratch && \
...
grub-mkstandalone \
    --format=i386-pc \
    --output=$PWD/$LIVE_BOOT/scratch/core.img \
...
cat \
    ...
    $PWD/$LIVE_BOOT/scratch/core.img \
> $PWD/$LIVE_BOOT/scratch/bios.img
